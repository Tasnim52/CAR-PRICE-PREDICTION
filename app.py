...
if models and ui:
    ...
    with col1:
        ...
        # CHANGE: We hide the algorithm selection from the UI but set it in the background
        # We automatically pick the first model available in your pickle file
        selected_algo = list(models.keys())[0]
    ...
    with col2:
        ...
        if st.button("CALCULATE ESTIMATED PRICE"):
            ...
            try:
                # The prediction still uses the 'selected_algo' variable set above
                prediction = models[selected_algo].predict(input_data)[0]

                # Display Result without mentioning the model name
                st.markdown(f"""
